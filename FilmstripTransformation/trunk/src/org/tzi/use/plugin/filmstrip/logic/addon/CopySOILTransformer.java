...
public class CopySOILTransformer {
	...
	public MStatement transformSoilOperationStatement(MStatement stmt, MOperation op){
	...
	private MStatement[] createCopyPrefix(MOperation op, List<VarDecl> vars){
		...
		vars.add(new VarDecl("s", snapshot.type()));
		vars.add(new VarDecl("op", opCls.type()));
		...
		try {
			selfSnapshot = new ExpNavigation(new ExpVariable("self", op.cls().type()), src, dst, Collections.<Expression>emptyList());
		...
		// op.aSelf := self
		stmtList.add(new MAttributeAssignmentStatement(new ExpVariable("op", opCls.type()), opCls.attribute(FilmstripModelConstants.OPC_SELF_VARNAME, true), new ExpVariable("self", op.cls().type())));
		// other parameters
		for(VarDecl var : op.paramList()){
			stmtList.add(new MAttributeAssignmentStatement(new ExpVariable("op", opCls.type()), opCls.attribute(var.name(), false), new ExpVariable(var.name(), var.type())));
		}
		...
		participants.add(new MRValueExpression(new ExpVariable("s", snapshot.type())));
		participants.add(new MRValueExpression(new ExpVariable("op", opCls.type())));
		stmtList.add(new MLinkInsertionStatement(filmstrip, participants, Collections.<List<MRValue>>emptyList()));
		
		// self := self.succ
		stmtList.add(new MVariableAssignmentStatement("self", new MRValueExpression(FilmstripUtil.handlePredSucc(new ExpVariable("self", op.cls().type()), false, new Stack<VarDeclList>()))));
	...
	private MStatement[] createCopySuffix(MOperation op) {
		...
		// retVal := result
		if(op.hasResultType()){
			Expression obj = new ExpVariable("op", opCls.type());
