 ...
 * 
 * based on {@link GEvalInstrTry_Assoc_LinkendSeqs}
 * 
 * @author Juergen Widdermann
 */
public class GEvalInstrTry_Assoc_LinkendSeqsKodKod extends GEvalInstruction
		implements IGCaller {
	 ...
	 */
	public GEvalInstrTry_Assoc_LinkendSeqsKodKod(
			GInstrTry_Assoc_LinkendSeqs instr, AsslTranslation asslTranslation, boolean backtrackingNecessary) {
		fInstr = instr;
	...
	public void eval(GConfiguration conf, IGCaller caller, IGCollector collector)
		...
		GCreatorKodKod.createFor(fLastEvaluatedInstruction).eval(conf, this,
				collector);
		fIterator.previous();
	}
	...
	public void feedback(GConfiguration conf, Value value, IGCollector collector)
		...
		if (fIterator.hasNext()) {
			fLastEvaluatedInstruction = (GInstruction) fIterator.next();
			GCreatorKodKod.createFor(fLastEvaluatedInstruction).eval(conf,
					this, collector);
			fIterator.previous();
		} else {
			if(backtrackingNecessary) {
	...
	protected void tryLinksKodKod(GConfiguration conf, IGCollector collector)
			throws GEvaluationException {
		crossProduct(fObjectLists, 0, null);
		fCaller.feedback(conf, null, collector);
	}

	@SuppressWarnings("unchecked")
	protected void crossProduct(List<List<MObject>> objects, int position,
			Vector<Object> entry) {
   		if (objects.size() > position) {
			List<MObject> objs = objects.get(position);
			for (MObject obj : objs) {
				Vector<Object> tmpList;
				if (entry == null) {
					tmpList = new Vector<Object>();
				} else {
					tmpList = (Vector<Object>) entry.clone();
				}
				tmpList.add(obj);
				crossProduct(objects, position + 1, tmpList);
			}
		} else {
			if (MAssociationClass.class.isInstance(fInstr.association())) {
				String tmpName = fInstr.association().name();
				String linkName = tmpName.substring(0, 1).toLowerCase() + tmpName.substring(1);
				asslTranslation.addObjectToUpperAssociationClassBound(fInstr.association().name(),
						linkName);
				entry.add(0,linkName);
				asslTranslation.addLinkToUpperAssociationClassBound(fInstr.association().name(),
						entry);
			} else {
				asslTranslation.addLinkToUpperAssociationBound(fInstr.association().name(),
						(Vector<MObject>) entry.clone());
			}
		}
	}
