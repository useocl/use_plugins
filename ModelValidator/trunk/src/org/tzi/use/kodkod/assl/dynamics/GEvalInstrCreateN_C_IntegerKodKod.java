 ...
 * eval CreateN
 * 
 * based on {@link GEvalInstrCreateN_C_Integer}
 * 
 * @author Juergen Widdermann
 * 
 */
public class GEvalInstrCreateN_C_IntegerKodKod extends GEvalInstruction
		implements IGCaller {
	 ...
	 */
	public GEvalInstrCreateN_C_IntegerKodKod(GInstrCreateN_C_Integer instr,
	...
	public void eval(GConfiguration conf, IGCaller caller, IGCollector collector)
		...
		collector.detailPrintWriter().println(
				new StringBuilder("evaluating `").append(fInstr).append("'")
						.toString());
		fCaller = caller;
		GCreatorKodKod.createFor(fInstr.integerInstr()).eval(conf, this, collector);
	}
	...
	public void feedback(
        ...
        for (String objectName : objectNames) {
    		...
    		// KodKod
    		//asslTranslation.addObjectToLowerClassBound(objectType.toString(), object.name());
        }
