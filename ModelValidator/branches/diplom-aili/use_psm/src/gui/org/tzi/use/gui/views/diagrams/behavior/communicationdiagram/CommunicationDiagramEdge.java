...
public class CommunicationDiagramEdge extends EdgeBase {
	...
	public MMessage getLongestMessage() {
		MMessage result = new MMessage();
		for (MMessage mess : messages) {
			if (mess.getSequenceNumberMessage().length() > result.getSequenceNumberMessage().length()) {
				result = mess;
			}
		}
		return result;
	...
	public int getPreferedLength() {
		...
		if (g == null) {
			return longestMessage.getSequenceNumberMessage().length();
		} else {
			return g.getFontMetrics().stringWidth(longestMessage.getSequenceNumberMessage()) * 2 + 100;
		}
	}
