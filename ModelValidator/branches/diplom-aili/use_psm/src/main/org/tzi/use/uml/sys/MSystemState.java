...
public final class MSystemState {
	...
	MSystemState(String name, MSystem system) {
		...
		derivedValuesController[0] = new DerivedLinkControllerDerivedEnd(this, fLinkSets);
		derivedValuesController[1] = new DerivedLinkControllerUnion(this, fLinkSets);
		derivedValuesController[2] = new DerivedAttributeController(this, fObjectStates);
			...
			if (linkSet.size() > 0) {
				...
				if (src instanceof MAssociationClass) {
					if (dst instanceof MAssociationClass) {
						// Links for a link object a stored with a single additional end of type MAssociationClass 
						throw new RuntimeException("Internal error! Wrong internal navigation from accociation class end to association class end.");
					}
					MLinkEnd linkEnd = ((MLinkObject)obj).getLinkEnd(dstIndex);
