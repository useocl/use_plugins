...
public class GModel extends MModel {
    ...
    public Collection<GFlaggedInvariant> loadedFlaggedInvariants(){
    	Collection<GFlaggedInvariant> ret = new LinkedList<GFlaggedInvariant>(fFlaggedInvariants.values());
    	Collection<MClassInvariant> modelInvariants = fModel.classInvariants();
    	
    	for (Iterator<GFlaggedInvariant> it = ret.iterator(); it.hasNext();) {
    		GFlaggedInvariant inv = it.next();
			if(modelInvariants.contains(inv.classInvariant())){
				it.remove();
			}
		}
